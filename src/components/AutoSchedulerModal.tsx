...
5. Return ONLY a valid JSON array of session objects.`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-zinc-950/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div
        className="bg-white border border-zinc-200/90 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden text-zinc-900 dark:bg-zinc-900 dark:border-zinc-800/90 dark:text-zinc-50"
        role="dialog"
        aria-modal="true"
      >
        {/* Header */}
        <header className="px-5 py-4 sm:px-6 sm:py-4.5 border-b border-zinc-100 flex items-center justify-between bg-gradient-to-r from-zinc-900 via-zinc-900 to-indigo-950 text-white shrink-0 dark:border-zinc-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-zinc-400 font-medium dark:text-zinc-500">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={onClose}
              className="p-1.5 rounded-xl text-zinc-400 hover:text-white hover:bg-zinc-800 transition-colors cursor-pointer dark:text-zinc-500"
              title="Close"
            >
              <X className="w-5 h-5" />
            </button>
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4">
          ...
          {showKeyInput && (
            <div className="p-3.5 bg-zinc-50 border border-zinc-200 rounded-xl space-y-2.5 animate-in fade-in dark:bg-zinc-800 dark:border-zinc-800">
              <div className="flex items-center justify-between">
                ...
                {currentApiKey && (
                  <button
                    ...
                    className="text-[10px] text-rose-600 hover:underline font-semibold cursor-pointer dark:text-rose-400"
                  >
                    Remove Saved Key
                  </button>
                )}
              ...
              <form onSubmit={handleSaveApiKey} className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-white border border-zinc-300 rounded-lg px-3 py-1.5 text-xs text-zinc-900 placeholder-zinc-400 focus:outline-none focus:ring-1 focus:ring-zinc-900 shadow-2xs font-mono dark:bg-zinc-900 dark:border-zinc-700 dark:text-zinc-50 dark:focus:ring-blue-500/40"
                />
                <button
                  type="submit"
                  ...
                  className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer ${
                    keyInputValue.trim()
                      ...
                      : 'bg-zinc-200 text-zinc-400 cursor-not-allowed dark:bg-zinc-600 dark:text-zinc-500'
                  }`}
                >
                  Save
                </button>
              ...
              {keySaveSuccess && (
                <p className="text-[11px] text-emerald-600 font-semibold flex items-center gap-1 dark:text-emerald-400">
                  <ShieldCheck className="w-3.5 h-3.5" />
                  Key successfully saved into localStorage!
                </p>
              )}

              <p className="text-[11px] text-zinc-500 leading-normal dark:text-zinc-400">
                Stored in browser <code className="bg-zinc-200/70 px-1 py-0.5 rounded text-[10px] dark:bg-zinc-600/70">localStorage.getItem('gemini_api_key')</code>. Direct requests are sent straight from your browser to Google Gen AI without backend server routes.
              </p>
            </div>
          )}
          ...
          {infoMessage && (
            <div className="p-3 bg-amber-50 border border-amber-200/80 rounded-xl flex items-start gap-2.5 text-xs text-amber-900 animate-in fade-in dark:bg-amber-500/10 dark:border-amber-500/30 dark:text-amber-300">
          ...
          {generatedEvents.length === 0 ? (
            <div className="space-y-4">
              <form onSubmit={handleSubmit} className="space-y-3">
                <label className="block text-xs font-bold text-zinc-700 uppercase tracking-wider dark:text-zinc-300">
                ...
                <div className="relative">
                  <textarea
                    ...
                    className="w-full bg-zinc-50 focus:bg-white text-zinc-900 placeholder-zinc-400 border border-zinc-300 focus:border-zinc-900 rounded-xl p-3.5 text-xs sm:text-sm outline-none focus:ring-2 focus:ring-zinc-900/10 transition-all leading-relaxed resize-none shadow-2xs dark:bg-zinc-800 dark:focus:bg-zinc-900 dark:text-zinc-50 dark:border-zinc-700 dark:focus:border-blue-500 dark:focus:ring-blue-500/30"
                    disabled={isLoading}
                    autoFocus
                  />
                  {goalPrompt && !isLoading && (
                    <button
                      ...
                      className="absolute right-3 top-3 text-zinc-400 hover:text-zinc-600 text-xs font-bold p-1 cursor-pointer dark:text-zinc-500 dark:hover:text-zinc-400"
                    >
                      Clear
                    </button>
                  )}
                ...
                <div className="space-y-1.5 pt-1">
                  <span className="text-[11px] font-semibold text-zinc-500 flex items-center gap-1 dark:text-zinc-400">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {SUGGESTED_CHIPS.map((chip, i) => (
                      <button
                        ...
                        disabled={isLoading}
                        className="text-[11px] text-zinc-600 hover:text-zinc-900 bg-zinc-100 hover:bg-zinc-200/80 border border-zinc-200/80 rounded-lg px-2.5 py-1.5 transition-colors cursor-pointer text-left active:scale-98 dark:text-zinc-400 dark:hover:text-zinc-50 dark:bg-zinc-700 dark:hover:bg-zinc-600/80 dark:border-zinc-800/80"
                      >
                        {chip}
                      </button>
                    ))}
                ...
                <div className="pt-2 flex items-center justify-end gap-2">
                  <button
                    ...
                    onClick={onClose}
                    disabled={isLoading}
                    className="px-4 py-2.5 rounded-xl border border-zinc-200 text-xs font-semibold text-zinc-600 hover:text-zinc-900 hover:bg-zinc-50 transition-colors cursor-pointer dark:border-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-50 dark:hover:bg-zinc-800"
                  >
                    Cancel
                  </button>
                  <button
              ...
              <div className="p-3.5 bg-sky-50/70 border border-sky-100 rounded-xl text-sky-950 flex items-start gap-2.5 dark:bg-sky-500/10 dark:border-sky-500/30 dark:text-sky-300">
                <CalendarIcon className="w-4 h-4 text-sky-600 shrink-0 mt-0.5 dark:text-sky-400" />
                <p className="text-xs leading-relaxed text-sky-800 dark:text-sky-300">
                  <strong className="font-semibold text-sky-900 dark:text-sky-300">Direct Client Intelligence: </strong>
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-200">
              ...
              <div className="flex items-center justify-between bg-zinc-50 border border-zinc-200/90 rounded-xl p-3 dark:bg-zinc-800 dark:border-zinc-800/90">
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center font-bold text-xs dark:bg-emerald-500/20 dark:text-emerald-300">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {activeSource === 'gemini' ? (
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-indigo-50 text-indigo-700 border border-indigo-200 dark:bg-indigo-500/10 dark:text-indigo-300 dark:border-indigo-500/30">
                          ⚡ Direct Gemini AI
                        </span>
                      ) : (
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300 dark:border-emerald-500/30">
                ...
                <button
                  type="button"
                  onClick={handleReset}
                  className="text-xs text-zinc-500 hover:text-zinc-800 font-semibold flex items-center gap-1 p-1 rounded hover:bg-zinc-100 cursor-pointer dark:text-zinc-400 dark:hover:text-zinc-200 dark:hover:bg-zinc-700"
                >
                  ...
                  return (
                    <div
                      ...
                      className={`p-3.5 rounded-xl border transition-all cursor-pointer flex items-start gap-3 select-none ${
                        isSelected
                          ? 'bg-white border-zinc-900/40 shadow-xs dark:bg-blue-500/10 dark:border-blue-500/40'
                          : 'bg-zinc-50/80 border-zinc-200 text-zinc-400 opacity-60 dark:bg-zinc-800/80 dark:border-zinc-800 dark:text-zinc-500'
                      }`}
                    >
                      {/* Checkbox */}
                      <div
                        className={`mt-0.5 w-4 h-4 rounded border flex items-center justify-center transition-colors shrink-0 ${
                          isSelected
                            ? 'bg-zinc-900 border-zinc-900 text-white dark:bg-blue-600 dark:border-blue-600'
                      ...
                      <div className="flex-1 min-w-0 space-y-1">
                        <div className="flex items-center justify-between gap-2 flex-wrap">
                          <h4
                            className={`text-xs sm:text-sm font-bold truncate ${
                              isSelected ? 'text-zinc-900 dark:text-zinc-50' : 'text-zinc-500 dark:text-zinc-400'
                            }`}
                          >
                            {evt.title}
                          </h4>
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            {/* Category Pill */}
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-zinc-100 text-zinc-700 border border-zinc-200 dark:bg-zinc-700 dark:text-zinc-300 dark:border-zinc-800">
                              {category}
                            </span>

                            {/* Duration Badge */}
                            <span className="text-[10px] font-semibold text-zinc-500 flex items-center gap-0.5 font-mono dark:text-zinc-400">
                        ...
                        {/* Description */}
                        {evt.description && (
                          <p className="text-xs text-zinc-500 leading-relaxed line-clamp-2 dark:text-zinc-400">
                            {evt.description}
                          </p>
                        )}

                        {/* Date & Countdown Info */}
                        <div className="pt-1 flex items-center gap-2 text-[11px] text-zinc-600 flex-wrap dark:text-zinc-400">
                          <span className="flex items-center gap-1 font-medium text-zinc-800 dark:text-zinc-200">
                            <CalendarIcon className="w-3 h-3 text-zinc-400 dark:text-zinc-500" />
                            {dateFormatted} {timeFormatted && `• ${timeFormatted}`}
                          </span>
                          <span className="text-zinc-300">•</span>
                          <span className="px-1.5 py-0.2 rounded-md bg-indigo-50 text-indigo-700 font-semibold border border-indigo-100 text-[10px] dark:bg-indigo-500/10 dark:text-indigo-300 dark:border-indigo-500/30">
                            ⏳ {relativeCountdown}
                          </span>
                        </div>
                      </div>
              ...
              <div className="pt-3 border-t border-zinc-100 flex items-center justify-between gap-3 dark:border-zinc-800">
                <button
                  type="button"
                  onClick={handleReset}
                  className="px-4 py-2.5 rounded-xl border border-zinc-200 text-xs font-semibold text-zinc-600 hover:text-zinc-900 hover:bg-zinc-50 transition-colors cursor-pointer dark:border-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-50 dark:hover:bg-zinc-800"
