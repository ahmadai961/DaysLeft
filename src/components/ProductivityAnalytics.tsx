...
export const ProductivityAnalytics: React.FC<ProductivityAnalyticsProps> = ({ tasks }) => {
  ...
  return (
    <section className="mt-12 pt-8 border-t border-zinc-800/90 text-zinc-50">
      {/* Section Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-xl sm:text-2xl font-extrabold tracking-tight text-zinc-50">
        ...
        <div className="flex items-center bg-zinc-700 p-1 rounded-xl border border-zinc-800/80 self-start sm:self-auto shrink-0 text-xs font-semibold">
          <button
            type="button"
            onClick={() => setTimeFilter('all')}
            className={`px-3 py-1.5 rounded-lg transition-all cursor-pointer ${
              timeFilter === 'all'
                ? 'bg-zinc-600 text-zinc-50 shadow-xs font-bold'
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="md:col-span-2 relative overflow-hidden rounded-2xl bg-gradient-to-br from-cyan-500/10 via-sky-500/10 to-zinc-900 border-2 border-cyan-500/30 p-5 sm:p-6 shadow-sm">
          ...
          <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="space-y-1.5 max-w-md">
              <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-cyan-500/20 border border-cyan-500/30 text-cyan-300 text-xs font-bold">
                <Zap className="w-3.5 h-3.5 text-cyan-400 fill-cyan-500" />
                <span>Time Saved Bank</span>
              </div>
              <h3 className="text-xl sm:text-2xl font-black text-cyan-300 tracking-tight">
                {totalTimeSavedMinutesOverall > 0
                  ? formatMinutesToFriendly(totalTimeSavedMinutesOverall)
                  : '0h 00m'}
                <span className="text-sm font-semibold text-cyan-300 ml-2">Total Saved</span>
              </h3>
            ...
            <div className="flex sm:flex-col gap-2 shrink-0">
              <div className="bg-zinc-900/80 backdrop-blur-xs border border-cyan-500/30 rounded-xl px-3.5 py-2 text-center shadow-2xs">
                <span className="text-xs text-cyan-300/80 font-semibold block">Early Completed</span>
              ...
              <div className="bg-zinc-900/80 backdrop-blur-xs border border-cyan-500/30 rounded-xl px-3.5 py-2 text-center shadow-2xs">
                <span className="text-xs text-cyan-300/80 font-semibold block">Pacing Status</span>
        ...
        <div className="bg-zinc-900 text-white rounded-2xl p-5 sm:p-6 flex flex-col justify-between shadow-sm border border-zinc-800">
          ...
          <div className="flex items-center justify-between pt-2 border-t border-zinc-800/80 text-xs text-zinc-500 font-medium">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5 sm:gap-4">
        {categoryStats.map((cat) => {
          ...
          return (
            <div
              key={cat.key}
              className={`rounded-2xl border ${cfg.borderLight} bg-zinc-900 p-4 sm:p-5 flex flex-col justify-between hover:shadow-md transition-all group`}
            >
              ...
              <div className="flex items-center justify-between gap-2 mb-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-zinc-50 group-hover:text-white">
                      {cfg.name}
                    </h4>
              ...
              {/* Footer Mini Stats */}
              <div className="flex items-center justify-between pt-3 mt-3 border-t border-zinc-800 text-[11px] text-zinc-400 font-medium">
