...
export const ProductivityAnalytics: React.FC<ProductivityAnalyticsProps> = ({ tasks }) => {
  ...
  return (
    <section className="mt-12 pt-8 border-t border-zinc-200/90 text-zinc-900">
      {/* Section Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center bg-zinc-100 p-1 rounded-xl border border-zinc-200/80 self-start sm:self-auto shrink-0 text-xs font-semibold">
          <button
            type="button"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="md:col-span-2 relative overflow-hidden rounded-2xl bg-gradient-to-br from-cyan-50 via-sky-50 to-white border-2 border-cyan-300 p-5 sm:p-6 shadow-sm">
          ...
          <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="space-y-1.5 max-w-md">
              <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-cyan-100/80 border border-cyan-200 text-cyan-900 text-xs font-bold">
                <Zap className="w-3.5 h-3.5 text-cyan-600 fill-cyan-500" />
                <span>Time Saved Bank</span>
              </div>
              <h3 className="text-xl sm:text-2xl font-black text-cyan-950 tracking-tight">
                {totalTimeSavedMinutesOverall > 0
                  ? formatMinutesToFriendly(totalTimeSavedMinutesOverall)
                  : '0h 00m'}
                <span className="text-sm font-semibold text-cyan-700 ml-2">Total Saved</span>
              </h3>
              <p className="text-xs text-cyan-800/80 leading-relaxed font-medium">
            ...
            <div className="flex sm:flex-col gap-2 shrink-0">
              <div className="bg-white/80 backdrop-blur-xs border border-cyan-200/80 rounded-xl px-3.5 py-2 text-center shadow-2xs">
                <span className="text-xs text-cyan-900/70 font-semibold block">Early Completed</span>
                <span className="text-base sm:text-lg font-mono font-black text-cyan-700">
                  {earlyCompletedTasksCount} {earlyCompletedTasksCount === 1 ? 'task' : 'tasks'}
                </span>
              </div>
              <div className="bg-white/80 backdrop-blur-xs border border-cyan-200/80 rounded-xl px-3.5 py-2 text-center shadow-2xs">
                <span className="text-xs text-cyan-900/70 font-semibold block">Pacing Status</span>
        ...
        <div className="bg-zinc-900 text-white rounded-2xl p-5 sm:p-6 flex flex-col justify-between shadow-sm border border-zinc-800">
          ...
          <div className="flex items-center justify-between pt-2 border-t border-zinc-800/80 text-xs text-zinc-400 font-medium">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5 sm:gap-4">
        {categoryStats.map((cat) => {
          ...
          return (
            <div
              key={cat.key}
              className={`rounded-2xl border ${cfg.borderLight} bg-white p-4 sm:p-5 flex flex-col justify-between hover:shadow-md transition-all group`}
            >
              ...
              <div className="flex items-center justify-between gap-2 mb-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-zinc-900 group-hover:text-zinc-950">
                      {cfg.name}
                    </h4>
              ...
              {/* Footer Mini Stats */}
              <div className="flex items-center justify-between pt-3 mt-3 border-t border-zinc-100 text-[11px] text-zinc-500 font-medium">
